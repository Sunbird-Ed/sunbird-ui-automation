...
const exploreChatBox = () => {
  ...
  try {
    ...
    content.closeChatBox.click();
    wait.waitForElementVisibility(content.clkChatBox, 30000, "clkChatBox button not available");
    content.clkChatBox.click();

    wait.waitForElementVisibility(content.chatlabel, 20000);
    expect(content.chatlabel.isDisplayed()).toBeTruthy();
    wait.waitForElementVisibility(content.chatDigitalContent, 20000);
    expect(content.chatDigitalContent.isDisplayed()).toBeTruthy();
    wait.waitForElementVisibility(content.chatCourse, 20000);
    expect(content.chatCourse.isDisplayed()).toBeTruthy();
    wait.waitForElementVisibility(content.chatDikshaMobileApp, 20000);
    expect(content.chatDikshaMobileApp.isDisplayed()).toBeTruthy();
    wait.waitForElementVisibility(content.chatcontentContribution, 20000);
    expect(content.chatcontentContribution.isDisplayed()).toBeTruthy();
    wait.waitForElementVisibility(content.chatOtherDIKSHAqueries, 20000);
    expect(content.chatOtherDIKSHAqueries.isDisplayed()).toBeTruthy();

    wait.waitForElementVisibility(content.chatDigitalContent, 30000, "chatDigitalContent button not available");
    content.chatDigitalContent.click();

    wait.waitForElementVisibility(content.chatExplore, 20000);
    expect(content.chatExplore.isDisplayed()).toBeTruthy();

    wait.waitForElementVisibility(content.closeChatBox, 30000, "clkChatBox button not available");
    content.closeChatBox.click();
  }
  catch (Exception) {
    console.log("Failed to see verify ChatBox");
  }
};
const exploreChatBOT = () => {
  ...
  try {
    ...
    wait.waitForElementVisibility(content.chatIGOT, 20000);
    expect(content.chatIGOT.isDisplayed()).toBeTruthy();
    wait.waitForElementVisibility(content.chatNishthaCourse, 20000);
    expect(content.chatNishthaCourse.isDisplayed()).toBeTruthy();
    wait.waitForElementVisibility(content.chatGoBack, 20000);
    expect(content.chatGoBack.isDisplayed()).toBeTruthy();
    wait.waitForElementVisibility(content.chatMainMenu2, 20000);
    expect(content.chatMainMenu2.isDisplayed()).toBeTruthy();

    wait.waitForElementVisibility(content.chatNishthaCourse, 30000, "chatTakeaCourse button not available");
    content.chatNishthaCourse.click();

    wait.waitForElementVisibility(content.verifykarnatakaState, 20000);
    expect(content.verifykarnatakaState.isDisplayed()).toBeTruthy();
    wait.waitForElementVisibility(content.closeChatBox, 30000, "clkChatBox button not available");
    content.closeChatBox.click();
  }
  catch (Exception) {
...
const searchedContentsDefaultALLtab = (contentData) => {
  var searchedMenu;
  try {
    ...
    content.searchInput.click();
    browser.sleep(1000);
    content.searchInput.sendKeys(contentData);
    browser.sleep(1000);
    content.searchIcon.click();
    browser.sleep(8000);
...
const consumeTextBook = (publishedTextBook) => {
  try {
    console.log("consuming the published content");
    browser.wait(protractor.ExpectedConditions.visibilityOf(sanityPage.SanityElement().clkLibraray), 20000, "clkLibraray  is not available");
    sanityPage.SanityElement().clkLibraray.click();
    browser.sleep(2000);
    sanityPage.SanityElement().searchConLib.click();
    browser.sleep(2000);
    sanityPage.SanityElement().searchConLib.sendKeys(publishedTextBook);
    browser.sleep(2000);
    sanityPage.SanityElement().clkSearchLib.click();
    browser.sleep(1000);
    content.courseCard.click();
    ...
    browser.sleep(2000);
    sanityPage.SanityElement().searchConLib.click();
    sanityPage.SanityElement().searchConLib.clear();
    browser.sleep(2000);
    sanityPage.SanityElement().searchConLib.sendKeys(do_id);
    browser.sleep(2000);
    sanityPage.SanityElement().clkSearchLib.click();
    ...
    console.log("Failed to consume the digitalTextbook properly.");
  }
}

const verifyFilterSortSearchInAllTextBook = (publishedTextBook) => {
  try {
    ...
    browser.sleep(5000);
    expect(searchObj.searchContent.isPresent()).toBeTruthy();
    expect(searchObj.showFilter1.isPresent()).toBeTruthy();
    expect(searchObj.sortBy.isPresent()).toBeTruthy();
    browser.sleep(2000);
    browser.wait(protractor.ExpectedConditions.visibilityOf(searchObj.showFilter1), 20000, "Book page not loaded");
    searchObj.showFilter1.click();
    browser.sleep(2000);
    expect(searchObj.contentType.isPresent()).toBe(false);
    browser.sleep(3000);
...
const verifyPublishedContentInAllMyContent = (publishedTextBook) => {
  try {
...
const validateDownloadToc = () => {
  try {
    ...
    browser.sleep(3000);
    expect(content.sharebutton.isPresent()).toBe(true);
...
const navigateContentAndValidate = (keywordNames, courseName) => {
  try {
    ...
    sanityPage.SanityElement().searchConLib.click();
    browser.sleep(2000);
    sanityPage.SanityElement().searchConLib.sendKeys(keywordNames);
    browser.sleep(2000);
    sanityPage.SanityElement().clkSearchLib.click();
    browser.sleep(1000);
    content.courseCard.getText().then(function (input) {
      expect(input).toMatch(courseName);
    })
  }
  catch (Exception) {
    console.log("Failed to verify navigate to Course section and search course");
  }
}


const epubContent = (courseName) => {
  try {
    browser.sleep(2000);
    console.log("User is trying to consume content");
    browser.wait(protractor.ExpectedConditions.visibilityOf(sanityPage.SanityElement().clkLibraray), 20000, "clkLibraray  is not available");
...
const consumeEpubContent = (courseName) => {
  try {
    browser.sleep(2000);
    console.log("User is trying to consume content");
    browser.wait(protractor.ExpectedConditions.visibilityOf(sanityPage.SanityElement().clkLibraray), 20000, "clkLibraray  is not available");
...
const verifyAndValidateEpubContent = () => {
  try {
    var sheetPath = getExcelPath.ConfigurePath().excelSheetPath;
    var cred = genericFun.readParticularDataFromExcelFile(sheetPath, '3');
    var contentName = cred[41]['TitleDescription'];
    browser.sleep(2000);
    console.log("User is trying to consume content");
    browser.wait(protractor.ExpectedConditions.visibilityOf(sanityPage.SanityElement().clkLibraray), 20000, "clkLibraray  is not available");
    sanityPage.SanityElement().clkLibraray.click();
    ...
    content.courseCard.click();
    browser.sleep(1000);
    content.pagenummberInput.click();
    content.pagenummberInput.clear();
    browser.sleep(100);
    content.pagenummberInput.sendKeys("9");
    browser.sleep(2000);
    content.rightArrowSearchbutton.click();
    browser.sleep(1000);
    content.assertPageNotFoundMsg.getText().then(function(input){
      var wrongPageNumMsg = "Page Not Found";
      console.log(input);
      expect(input).toMatch(wrongPageNumMsg);
    })
    browser.sleep(1000);
    ...
    browser.sleep(1000);
    content.assertPercentageOfCompletion.getText().then(function (input) {
      console.log("Completion Percentage is " + input);
    })
...
const consumeContetnByKeyBoardevents = () => {
  try {
    var sheetPath = getExcelPath.ConfigurePath().excelSheetPath;
    var cred = genericFun.readParticularDataFromExcelFile(sheetPath, '3');
    var do_id = cred[34]['CourseName'];
    ...
    browser.sleep(2000);
    searchObj.closeShareButton.click();
    browser.sleep(2000);
    //browser.actions().sendKeys(protractor.Key.TAB);
    //browser.actions().sendKeys(protractor.Key.ENTER);
    //searchObj.joinCourseButton.sendKeys(protractor.Key.TAB, protractor.Key.TAB, protractor.Key.TAB, protractor.Key.TAB);
    //searchObj.joinCourseButton.sendkeys(protractor.key.TAB);
    // browser.sleep(2000);
    searchObj.joinCourseButton.sendkeys(protractor.Key.ENTER);

    
    // browser.sleep(5000);
    // content.courseClk.sendKeys(protractor.key.ENTER);
    // browser.sleep(9000);
    // for (i = 0; i > 2; i++) {
    //   content.zoomOutToc.sendKeys(protractor.key.ENTER);
    //   browser.sleep(1000);
    // }
    // for (i = 0; i < 94; i++) {
    //   sanityPage.SanityElement().pdfArrowButton.sendKeys(protractor.key.ENTER);
    //   browser.sleep(200);
    // }
    // sanityPage.SanityElement().pdfArrowButton.sendKeys(protractor.key.ENTER);
    // browser.sleep(5000);
...
const validatePrintButtonInplayer = () => {
  try {
    var sheetPath = getExcelPath.ConfigurePath().excelSheetPath;
    var cred = genericFun.readParticularDataFromExcelFile(sheetPath, '3');
    var do_id = cred[52]['CourseName'];
    browser.sleep(2000);
    console.log("User is trying to consume content");
    browser.wait(protractor.ExpectedConditions.visibilityOf(sanityPage.SanityElement().clkLibraray), 20000, "clkLibraray  is not available");
    sanityPage.SanityElement().clkLibraray.click();
    ...
    content.courseCard.click();
    browser.sleep(1000);
    content.expandAllButton.click();
    browser.sleep(1000);
    content.clkCoursePdf.click();
    browser.sleep(9000);
    browser.actions().dragAndDrop(content.overlayButton, content.overlayButton).mouseUp().perform();
    browser.sleep(3000);
    expect((content.printButtonInPlayerNavBar).isPresent()).toBe(true);
    browser.sleep(3000);
    browser.actions().dragAndDrop(content.overlayButton, content.overlayButton).mouseUp().perform();
    browser.sleep(6000);
  }
...
const validateLabelPageIsChangedFromLocation = () => {
  try {
    var sheetPath = getExcelPath.ConfigurePath().excelSheetPath;
    var cred = genericFun.readParticularDataFromExcelFile(sheetPath, '3');
    var do_id = cred[52]['CourseName'];
    ...
    sanity.pdfArrowButton.click();
    browser.sleep(2000);
    expect((content.assertPagesReplaceWithLocationLabel).isPresent()).toBe(true);
    browser.sleep(2000);
