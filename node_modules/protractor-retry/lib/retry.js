...
function afterLaunch(configRetry) {
    ...
    */
    retry = (configRetry) ? configRetry : 0;
...
function resultsFilePath(retry) {
    var resultsDir = path.resolve(process.cwd(), 'projectFailuresReport');
    var fileName = 'testErrors';
...
async function writeToFile(files) {
    ...
    if (files && files.length > 0) {
        ...
        txt = prepareData(config.specs.toString());
    }
    if (!fs.existsSync(file)) {
