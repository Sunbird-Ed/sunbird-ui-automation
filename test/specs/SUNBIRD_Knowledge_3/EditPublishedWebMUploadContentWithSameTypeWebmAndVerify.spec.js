...
describe('upload pdf, save and send for review and publish.', () =>{
    ...
    it('EditPublishedWebMUploadContentWithSameTypeWebmAndVerify ',function(){
        utility.handleDropDown();
        utility.handleLocationPopup();
        utility.userLogin('Creator');
        var sheetPath = getExcelPath.ConfigurePath().excelSheetPath;
        var cred = genericFun.readParticularDataFromExcelFile(sheetPath, "5");
        var Webm = cred[3]["CourseDescription"];

        sanityFun.validateContentInPublishedSectionAfterPublishmentOfContent(Webm);
        uploadPageObj.ReuploadSameTypeForLiveUploadContent('WEBM');
        utility.userLogout();
         utility.userLogin('Reviewer');
            resourcePageObj.publishTheResourceFromUpForReview(Webm);
